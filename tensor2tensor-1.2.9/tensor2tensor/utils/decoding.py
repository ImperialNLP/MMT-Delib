...
def _get_sorted_inputs(filename, firstPfilename, imagePfilename, num_shards=1, delimiter="\n", img_idx=None):
  ...
  Args:
    ...
    firstPfilename: path to file with firstP, 1 per line.
...
def _decode_input_tensor_to_features_dict(feature_map, hparams):
  ...
  Args:
    feature_map: a dictionary with keys `problem_choice`, `input` and `firstP` containing Tensors.
    hparams: model hyperparameters
